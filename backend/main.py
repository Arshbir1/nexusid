...
# Import auth so UserDB is registered with Base metadata
from backend.core.auth import UserDB, seed_users
...
def _ensure_users(db: Session):
    """Ensure default users exist (idempotent). Creates table if needed."""
    from backend.core.auth import UserDB, seed_users
    try:
...
def login(request_body: dict, db: Session = Depends(get_db)):
    ...
    from backend.core.auth import UserDB, verify_password, create_token
    _ensure_users(db)  # Auto-seed on first login attempt
    ...
    return {
        ...
        "role": user.role, "expires_in": expires_in,
    }


@app.get("/api/auth/verify")
def verify_token(credentials=Depends(HTTPBearer(auto_error=False)), db: Session = Depends(get_db)):
    """Verify if a token is still valid."""
    from backend.core.auth import decode_token, UserDB
    if not credentials:
        return {"valid": False}
    try:
        ...
        if user and user.is_active:
            return {"valid": True, "username": user.username, "full_name": user.full_name, "role": user.role}
        return {"valid": False}
    except Exception:
        return {"valid": False}
...
def list_users(db: Session = Depends(get_db)):
    """List all users."""
    from backend.core.auth import UserDB
    users = db.query(UserDB).all()
    return [{"username": u.username, "full_name": u.full_name, "role": u.role,
             "is_active": u.is_active, "last_login": u.last_login.isoformat() if u.last_login else None} for u in users]
